...
def select_files_by_level(filen_path_format, replace_value, rule_exact=False, replace_marks="{LEVEL}"):
    file_names = []
    if rule_exact:
        rule_file = filen_path_format.replace(replace_marks, f"{replace_value}")
        ...
        for level in range(replace_value + 1):
            rule_file = filen_path_format.replace(replace_marks, f"{level}")
            if file_is_exist(rule_file):
                file_names.append(rule_file)
    return file_names
